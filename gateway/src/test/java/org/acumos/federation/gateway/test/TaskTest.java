...
@SpringBootTest(classes = org.acumos.federation.gateway.Application.class, webEnvironment = WebEnvironment.RANDOM_PORT, properties = {
		"federation.instance=adapter", "federation.instance.name=test",
		...
		"federation.ssl.client-auth=need", "peersLocal.source=classpath:/task-test-peers.json",
...
public class TaskTest {
	...
	/**
	 * Called by the test framework and hanging until the event that we are
	 * expecting is coming. The test config sets up a short check interval.
	 */
	@Test
	public void testPeerTask() {

		PeerSubscriptionListener listener = (PeerSubscriptionListener) this.context.getBean("testListener");

		try {
			BasicHttpResponse mockResponse = new BasicHttpResponse(
					new BasicStatusLine(new ProtocolVersion("HTTP", 1, 1), 200, "Success"));
			/*
			 * String mockContent = "{" + "\"status\": true," + "\"response_code\": 200," +
			 * "\"response_detail\": \"Success\"," + "\"response_body\": [{" +
			 * "\"solutionId\":\"6793411f-c7a1-4e93-85bc-f91d267541d8\"," +
			 * "\"name\":\"mock model\"," + "\"description\":\"Test mock model\"," +
			 * "\"ownerId\":\"admin\"," + "\"active\":\"true\"," +
			 * "\"modelTypeCode\":\"CL\"," + "\"toolkitTypeCode\":\"\"," +
			 * "\"validationStatusCode\":\"\"," + "\"metadata\":\"acumosa\"," +
			 * "\"created\":\"2017-08-10\"," + "\"modified\":\"2017-08-11\"" + "}]}";
			 * 
			 * byte[] mockContentBytes = mockContent.getBytes("UTF-8");
			 * 
			 * mockResponse.setEntity( new ByteArrayEntity(mockContentBytes));
			 * mockResponse.addHeader("Content-Type",
			 * ContentType.APPLICATION_JSON.toString());
			 * mockResponse.addHeader("Content-Length",
			 * String.valueOf(mockContentBytes.length));
			 */
			ClassPathResource mockResource = new ClassPathResource("mockPeerSolutionsResponse.json");

			mockResponse.setEntity(new InputStreamEntity(mockResource.getInputStream()));
			...
			when(this.federationClient.execute(any(HttpHost.class), any(HttpRequest.class))).thenReturn(mockResponse);

			when(this.federationClient.execute(any(HttpHost.class), any(HttpRequest.class), any(HttpContext.class)))
					.thenReturn(mockResponse);

			when(this.federationClient.execute(any(HttpUriRequest.class))).thenReturn(mockResponse);
			...
			when(this.federationClient.execute(any(HttpHost.class), any(HttpRequest.class), any(ResponseHandler.class)))
					.thenReturn(mockPayload);

			when(this.federationClient.execute(any(HttpHost.class), any(HttpRequest.class), any(ResponseHandler.class),
					any(HttpContext.class))).thenReturn(mockPayload);

			when(this.federationClient.execute(any(HttpUriRequest.class), any(ResponseHandler.class)))
					.thenReturn(mockPayload);
		...
		try {
			...
			assertTrue("All expected events have occured in the test interval", complete);
