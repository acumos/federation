...
/**
 * Infrastrcture code for mocking underlying REST calls, such in the case of CDS
 * api calls.
 */
public abstract class ServiceTest {

	// needed because service calls to other components are made over the local
	// interface
	@MockBean
	...
	public void initTest() throws IOException {
		...
		when(this.localClient.execute(any(HttpUriRequest.class), any(HttpContext.class)))
				.thenAnswer(new Answer<HttpResponse>() {
					public HttpResponse answer(InvocationOnMock theInvocation) throws Throwable {
						...
						String key = req.getMethod() + " " + req.getURI().getPath()
								+ (req.getURI().getQuery() == null ? "" : ("?" + req.getURI().getQuery()));
						...
						BasicCloseableHttpResponse httpResponse = new BasicCloseableHttpResponse(
								new BasicStatusLine(new ProtocolVersion("HTTP", 1, 1), mockResponse.getResponseCode(),
										mockResponse.getResponseMsg()));
		...
		when(this.localConfig.buildClient()).thenAnswer(new Answer<HttpClient>() {
	...
	/**
	 * Apache's http framework expects a CloseableHttpResponse at some point and
	 * this is cheaper than mocking every time.
	 */
