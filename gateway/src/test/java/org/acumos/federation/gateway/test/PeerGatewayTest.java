...
public class PeerGatewayTest {
	...
	public void testGateway() {
		...
		try {
			...
			BasicHttpResponse mockSolutionRevisionsResponse = new BasicHttpResponse(
					new BasicStatusLine(new ProtocolVersion("HTTP", 1, 1), 200, "Success"));

			ClassPathResource mockSolutionRevisions = new ClassPathResource("mockPeerSolutionRevisionsResponse.json");

			mockSolutionRevisionsResponse.setEntity(new InputStreamEntity(mockSolutionRevisions.getInputStream()));
			mockSolutionRevisionsResponse.addHeader("Content-Type", ContentType.APPLICATION_JSON.toString());
			mockSolutionRevisionsResponse.addHeader("Content-Length",
					String.valueOf(mockSolutionRevisions.contentLength()));

			BasicHttpResponse mockSolutionRevisionArtifactsResponse = new BasicHttpResponse(
					new BasicStatusLine(new ProtocolVersion("HTTP", 1, 1), 200, "Success"));

			ClassPathResource mockSolutionRevisionArtifacts = new ClassPathResource(
					"mockPeerSolutionRevisionArtifactsResponse.json");

			mockSolutionRevisionArtifactsResponse
					.setEntity(new InputStreamEntity(mockSolutionRevisionArtifacts.getInputStream()));
			mockSolutionRevisionArtifactsResponse.addHeader("Content-Type", ContentType.APPLICATION_JSON.toString());
			mockSolutionRevisionArtifactsResponse.addHeader("Content-Length",
					String.valueOf(mockSolutionRevisionArtifacts.contentLength()));

			BasicHttpResponse mockDownloadResponse = new BasicHttpResponse(
					new BasicStatusLine(new ProtocolVersion("HTTP", 1, 1), 200, "Success"));

			mockDownloadResponse.setEntity(new ByteArrayEntity(new byte[] {}));
			mockDownloadResponse.addHeader("Content-Type", ContentType.APPLICATION_OCTET_STREAM.toString());
			mockDownloadResponse.addHeader("Content-Length", String.valueOf(0));

			// prepare the clients
			when(this.clients.getCDSClient())
			...
			when(this.clients.getFederationClient(any(String.class))).thenAnswer(new Answer<FederationClient>() {
				public FederationClient answer(InvocationOnMock theInvocation) {
					...
					// client
					return new FederationClient((String) theInvocation.getArguments()[0]/* the URI */,
							federationClient);
					/*
					 * not working as real method relies on the application context which is not set
					 * because we work on a mock try { return
					 * (FederationClient)theInvocation.callRealMethod(); } catch (Throwable t) {
					 * t.printStackTrace(); return null; }
					 */
				}
			});

			when(this.clients.getNexusClient()).thenReturn(nexusClient);

			when(this.nexusClient.uploadArtifact(any(String.class), any(String.class), any(String.class),
					any(String.class), any(Long.class), any(InputStream.class)))
							.thenReturn(new UploadArtifactInfo("", "", "", "", "", 0));

			when(this.cdsClient.searchPeers(any(Map.class), any(Boolean.class), any(RestPageRequest.class)))
					.thenAnswer(new Answer<RestPageResponse<MLPPeer>>() {
						public RestPageResponse<MLPPeer> answer(InvocationOnMock theInvocation) {
							...
							if (selector != null && selector.containsKey("isSelf")
									&& selector.get("isSelf").equals(Boolean.TRUE)) {
								peer.setPeerId("0");
			...
			when(this.cdsClient.getPeers(any(RestPageRequest.class)))
					.thenAnswer(new Answer<RestPageResponse<MLPPeer>>() {
						public RestPageResponse<MLPPeer> answer(InvocationOnMock theInvocation) {
							MLPPeer peer = new MLPPeer();
							peer.setPeerId("1");
			...
			when(this.cdsClient.getPeerSubscriptions(any(String.class)))
			...
			when(this.cdsClient.getSolution(any(String.class))).thenReturn(null);
			...
			when(this.cdsClient.createSolution(any(MLPSolution.class)))
			...
			when(this.cdsClient.getSolutionRevisions(any(String.class)))
			...
			when(this.cdsClient.createSolutionRevision(any(MLPSolutionRevision.class)))
			...
			when(this.cdsClient.getArtifact(any(String.class))).thenReturn(null);

			when(this.cdsClient.createArtifact(any(MLPArtifact.class))).thenAnswer(new Answer<MLPArtifact>() {
			...
			}).when(this.cdsClient).addSolutionRevisionArtifact(any(String.class), any(String.class),
					any(String.class));
