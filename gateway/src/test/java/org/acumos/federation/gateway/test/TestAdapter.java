...
public class TestAdapter {
	...
	public void initTestAdapter() {
		log.debug(EELFLoggerDelegate.debugLogger, "initTestAdapter");
		...
		// Done
		log.debug(EELFLoggerDelegate.debugLogger, "TestAdapter available");
	}

	@PreDestroy
	public void cleanupTestAdapter() {
		log.debug(EELFLoggerDelegate.debugLogger, "TestAdapter destroyed");
	}
			...
			synchronized (peerImports) {
				for (MLPSolution solution : this.solutions) {
					...
					try {
						...
						log.warn(EELFLoggerDelegate.debugLogger, "Failed to retrieve revisions", x);
						continue;
					}
					...
					try {
						...
						log.warn(EELFLoggerDelegate.debugLogger, "Failed to retrieve artifacts", x);
						continue;
					}
					log.debug(EELFLoggerDelegate.debugLogger,
					...
					for (MLPArtifact artifact : artifacts) {
						...
						try {
							...
							log.debug(EELFLoggerDelegate.debugLogger, "Failed to download artifact", x);
						}
					}
				}
			}
