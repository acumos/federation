...
import org.acumos.federation.client.DeployedModelClient;
...
public class DeployedModelServiceImpl implements DeployedModelService {
	...
	public void updateModelParams(String ingressUrl, JsonNode params) throws RestClientException{
		DeployedModelClient deployedModelClient = new DeployedModelClient(ingressUrl, new ClientConfig());
	...
	public String updateParamsforAllDeployments(ModelData payload) throws IOException {
		...
		ICommonDataServiceRestClient cdsClient = clients.getCDSClient();
		...
		for (MLPSolutionDeployment mlpSolutionDeployment : mlpSolutionDeployments) {
			boolean isUpdateSuccess = updateParamsForDeployment(payload, totalFailure, mlpSolutionDeployment);
			if (!isUpdateSuccess){
				totalFailure++;
			}
		}
		...
		if(totalFailure > 0){
			responseMessage = String.format("Params data posted to deployed models,of total {} " +
							"deployments failed posting to {} deployments"
					,mlpSolutionDeployments.size(),totalFailure);
		}else {
			responseMessage = String.format("Params data posted to deployed models, total {} deployments updated"
					,mlpSolutionDeployments.size());
		}
		return responseMessage;
	}

	private boolean updateParamsForDeployment(ModelData payload, int totalFailure, MLPSolutionDeployment mlpSolutionDeployment) throws IOException {
		...
		if (isContinousTrainingEnabled) {
			...
			try {
				...
				log.error("successfully posted params to deployment with deployment id {} , solution id {} " +
								", revision id {}",
