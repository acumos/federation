...
public class Clients {
	 ...
	 * notification when one is deleted (or has it's API URL changed).
	 * It would have been possible to keep federation clients in a hash
	 * and fault them in, as needed, but, without a means for identifying
	 * clients that were no longer needed, that would have constituted
	 * a memory (and possibly a TCP/IP connection) leak.  So this code
	 * does not cache federation clients.
	 *
	 * For some reason, the DockerClient seems to go stale, resulting
	 * in operations (like the docker pull command) unexpectedly hanging,
	 * with no error or indication of a problem.  Creating a fresh
	 * DockerClient on each upload/download of a Docker image artifact,
	 * as a workaround, seems to work.
	 */
