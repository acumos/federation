...
public class CatalogServiceImpl implements CatalogService	{
	...
	public MLPSolution createSolution(MLPSolution solution) {
		Set<MLPTag> tags = solution.getTags();
		solution.setTags(Collections.emptySet());
		solution = clients.getCDSClient().createSolution(solution);
	...
	public void updateSolution(MLPSolution solution) {
		doTags(solution.getTags(), solution.getSolutionId());
		solution.setTags(Collections.emptySet());
		clients.getCDSClient().updateSolution(solution);
	}

	private void doTags(Set<MLPTag> tags, String solutionId) {
		for (MLPTag tag: tags) {
			clients.getCDSClient().addSolutionTag(solutionId, tag.getTag());
		}
	}
