...
public class ONAP {
	...
	public void initOnap() {
		log.debug(EELFLoggerDelegate.debugLogger, "initOnap");
		...
		// Done
		log.debug(EELFLoggerDelegate.debugLogger, "Onap available");
	}

	@PreDestroy
	public void cleanupOnap() {
		log.debug(EELFLoggerDelegate.debugLogger, "Onap destroyed");
	}
	...
	public class ONAPPushTask implements Runnable {
		...
		public void run() {
			...
			try {
				...
				log.warn(EELFLoggerDelegate.debugLogger, "Failed to list ONAP SDC assets: " + x.getCause(), x);
		...
		public JSONObject createSdcAsset(MLPSolution theSolution) throws Exception {
			...
			try {
				...
				log.warn(EELFLoggerDelegate.debugLogger, "Failed to create ONAP SDC VF", x);
				throw x;
			}
		}
		...
		public void updateAssetArtifacts(JSONObject theAssetInfo, MLPSolution theSolution) throws Exception {
			...
			try {
				...
				log.warn(EELFLoggerDelegate.errorLogger, "Failed to retrieve acumos revisions: " + x);
				throw x;
			}
			...
			try {
				...
				log.warn(EELFLoggerDelegate.debugLogger, "Failed to retrieve acumos artifacts" + x);
				throw x;
			}
			...
			log.warn(EELFLoggerDelegate.debugLogger, "New SDC artifacts: " + newArtifacts);
			for (MLPArtifact acumosArtifact : newArtifacts) {
				try {
					...
					log.warn(EELFLoggerDelegate.debugLogger, "Failed to create ONAP SDC VF Artifact", x);
				}
			}
			...
			for (Map.Entry<MLPArtifact, JSONObject> updateEntry : updatedArtifacts.entrySet()) {
				...
				try {
					...
					log.warn(EELFLoggerDelegate.debugLogger, "Failed to update ONAP SDC VF Artifact", x);
				}
			}
			...
			for (JSONObject sdcArtifact : deletedArtifacts) {
				try {
					...
					log.warn(EELFLoggerDelegate.debugLogger, "Failed to delete ONAP SDC VF Artifact", x);
				}
			}
		}
		...
		for (int i = 0; i < sdcAssets.length(); i++) {
			...
			if (state != null && "NOT_CERTIFIED_CHECKEOUT".equals(state)) {
				try {
					...
					log.info(EELFLoggerDelegate.debugLogger, "Cleanup ONAP SDC asset: " + sdcAsset.optString("uuid"),
							x);
				}
			}
		}
