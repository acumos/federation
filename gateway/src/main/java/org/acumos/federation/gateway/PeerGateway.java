...
public class PeerGateway	{
	private static final Logger log = LoggerFactory.getLogger(PeerGateway.class);
	@Autowired
	...
	private class PeerSubscriptionPoller implements Runnable {
		...
		private void checkSolution(String solutionId, String catalogId, boolean inLocalCatalog, FederationClient peer) {
			...
			if (isnew) {
				...
				pSol.getTags().removeAll(lSol.getTags());
				changed |= !pSol.getTags().isEmpty();
		...
		private void checkSubscription() {
			...
			userId = subscription.getUserId() != null? subscription.getUserId(): federation.getOperator();
